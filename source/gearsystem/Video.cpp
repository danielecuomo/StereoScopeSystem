...
unsigned int Video::GetCyclesToNextEvent() const
{
    unsigned int current = static_cast<unsigned int>(m_iCycleCounter);
    unsigned int next = GS_CYCLES_PER_LINE;

    const auto consider = [&](bool done, int timing, bool strict_greater) {
        if (done)
            return;
        unsigned int target = static_cast<unsigned int>(timing);
        if (strict_greater)
            target += 1;
    ...
    consider(m_LineEvents.vint, m_Timing[TIMING_VINT], true);
    consider(m_LineEvents.display, m_Timing[TIMING_DISPLAY], false);
    consider(m_LineEvents.scrollx, m_Timing[TIMING_XSCROLL], false);
    consider(m_LineEvents.hint, m_Timing[TIMING_HINT], false);
    consider(m_LineEvents.vcounter, m_Timing[TIMING_VCOUNT], false);
    consider(m_LineEvents.vintFlag, m_Timing[TIMING_FLAG_VINT], false);
    ...
    if (m_bSpriteCollisionRequest && !m_LineEvents.render)
    {
        ...
        if (col_cycle > static_cast<int>(current) && static_cast<unsigned int>(col_cycle) < next)
            next = static_cast<unsigned int>(col_cycle);
    }
...
{
    const int y_offset = m_bExtendedMode224 ? GS_RESOLUTION_GG_Y_OFFSET_EXTENDED : GS_RESOLUTION_GG_Y_OFFSET;
    const int scy_adjust = (m_bGameGear && !m_bGameGearSMSMode) ? y_offset : 0;
    const int scy = line;
    const int line_width_info = line * (m_iScreenWidth - m_iHideLeftBarOffset);
    const int line_width_screen = (line - scy_adjust) * (m_iScreenWidth - m_iHideLeftBarOffset);
    int origin_x = m_ScrollX;
    if ((line < 16) && IsSetBit(m_VdpRegister[0], 6))
        origin_x = 0;
    const int origin_y = m_ScrollY;
    ...
    const int scx_begin = (m_bGameGear && !m_bGameGearSMSMode) ? GS_RESOLUTION_GG_X_OFFSET : m_iHideLeftBarOffset;
    const int scx_end = scx_begin + m_iScreenWidth - m_iHideLeftBarOffset;

    // The old implementation recomputed tile address, tile attributes and the
    // four pattern bytes for every pixel. A scanline is 32 tile cells wide, so
    // retain those values for all 8 pixels belonging to the same tile row.
    int cached_tile_x = -1;
    int cached_tile_y = -1;
    int cached_tile_addr = -1;
    int cached_row = -1;
    int cached_palette_offset = 0;
    bool cached_priority = false;
    bool cached_hflip = false;
    u8 cached_pattern[4] = {0, 0, 0, 0};

    for (int scx = scx_begin; scx < scx_end; ++scx)
    {
        const int scx_diff = scx - scx_begin;
        const int pixel_screen = line_width_screen + scx_diff;
        const int pixel_info = line_width_info + scx_diff;

        if (line < max_height)
        {
            if (IsSetBit(m_VdpRegister[0], 5) && scx < 8)
            {
                m_pFrameBuffer[pixel_screen] = ColorFromPalette((m_VdpRegister[7] & 0x0F) + 16);
                m_pInfoBuffer[pixel_info] = 0;
                continue;
            }

            int effective_map_y = map_y;
            if (IsSetBit(m_VdpRegister[0], 7) && scx >= 192)
                effective_map_y = scy;

            const int effective_tile_y = effective_map_y >> 3;
            const int effective_tile_y_offset = effective_map_y & 7;
            const u8 map_x = static_cast<u8>(scx - origin_x);
            const int tile_x = map_x >> 3;
            const int tile_x_offset = map_x & 7;

            const int effective_row = effective_tile_y_offset;

            if (tile_x != cached_tile_x || effective_tile_y != cached_tile_y || effective_row != cached_row)
            {
                cached_tile_x = tile_x;
                cached_tile_y = effective_tile_y;
                cached_row = effective_row;

                const u16 tile_addr = map_address + (((effective_tile_y << 5) + tile_x) << 1);
                const u8 tile_lo = m_pVdpVRAM[tile_addr];
                const u8 tile_info = m_pVdpVRAM[tile_addr + 1];
                int tile_index = tile_lo;
                if (IsSetBit(tile_info, 0))
                    tile_index = (tile_index | 0x0100) & 0x1FF;

                cached_hflip = IsSetBit(tile_info, 1);
                const bool vflip = IsSetBit(tile_info, 2);
                cached_palette_offset = IsSetBit(tile_info, 3) ? 16 : 0;
                cached_priority = IsSetBit(tile_info, 4);

                const int row = vflip ? 7 - effective_row : effective_row;
                cached_tile_addr = (tile_index << 5) + (row << 2);
                cached_pattern[0] = m_pVdpVRAM[cached_tile_addr + 0];
                cached_pattern[1] = m_pVdpVRAM[cached_tile_addr + 1];
                cached_pattern[2] = m_pVdpVRAM[cached_tile_addr + 2];
                cached_pattern[3] = m_pVdpVRAM[cached_tile_addr + 3];
            }

            int tile_pixel_x = cached_hflip ? tile_x_offset : 7 - tile_x_offset;
            const int bit = tile_pixel_x;
            const int palette_color =
                ((cached_pattern[0] >> bit) & 0x01) |
                (((cached_pattern[1] >> bit) & 0x01) << 1) |
                (((cached_pattern[2] >> bit) & 0x01) << 2) |
                (((cached_pattern[3] >> bit) & 0x01) << 3) |
                cached_palette_offset;

            const bool final_priority = cached_priority && ((palette_color - cached_palette_offset) != 0);
...
void Video::RenderSpritesSMSGG(int line)
{
    ...
    for (int i = render_sprite_count - 1; i >= 0; i--)
    {
        ...
        const int sprite_tile_addr = sprite_tiles_address + (sprite_tile << 5) +
            (((line - sprite_y) >> (sprite_zoom ? 1 : 0)) << 2);
        const u8 sprite_pattern0 = m_pVdpVRAM[sprite_tile_addr + 0];
        const u8 sprite_pattern1 = m_pVdpVRAM[sprite_tile_addr + 1];
        const u8 sprite_pattern2 = m_pVdpVRAM[sprite_tile_addr + 2];
        const u8 sprite_pattern3 = m_pVdpVRAM[sprite_tile_addr + 3];
        ...
        for (int tile_x = 0; tile_x < sprite_width; tile_x++)
        {
            ...
            int palette_color = ((sprite_pattern0 >> tile_pixel_x) & 0x01) +
                    (((sprite_pattern1 >> tile_pixel_x) & 0x01) << 1) +
                    (((sprite_pattern2 >> tile_pixel_x) & 0x01) << 2) +
                    (((sprite_pattern3 >> tile_pixel_x) & 0x01) << 3);
            if (palette_color == 0)
                continue;
...
void Video::Render16bit(u16* srcFrameBuffer, u8* dstFrameBuffer, GS_Color_Format pixelFormat, int size, bool overscan)
{
    ...
    // Native SMS RGB565 conversion: the core framebuffer stores palette
    // indices, so use a tiny palette lookup instead of doing bit extraction
    // and three LUT accesses for every pixel.
    if (!overscan && !m_bGameGear && !m_bTMS9918 &&
        pixelFormat == GS_PIXEL_RGB565 &&
        size == GS_RESOLUTION_SMS_WIDTH * GS_RESOLUTION_SMS_HEIGHT)
    {
        // The SMS framebuffer contains the 6-bit color value itself, not
        // the CRAM slot number. Build the 64 possible RGB565 values directly
        // from that value. Indexing m_pVdpCRAM here would incorrectly treat
        // the color value as a palette address and produces false colors.
        u16 pal565[64];
        for (int i = 0; i < 64; ++i)
        {
            const u8 r = i & 0x03;
            const u8 g = (i >> 2) & 0x03;
            const u8 b = (i >> 4) & 0x03;
            pal565[i] = (u16)((k2bitTo5bit[r] << 11) |
                              (k2bitTo6bit[g] << 5) |
                              k2bitTo5bit[b]);
        }
        u16* dst16 = reinterpret_cast<u16*>(dstFrameBuffer);
        for (int i = 0; i < size; ++i)
            dst16[i] = pal565[srcFrameBuffer[i] & 0x3F];
        return;
    }
