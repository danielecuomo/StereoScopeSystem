...
inline void Audio::Tick(unsigned int clockCycles)
{
    ...
    // YM2413::Tick is only an elapsed-cycle accumulator; avoid the call when
    // the FM chip is disabled (the normal SMS case).
    if (m_bYM2413Enabled && !m_bYM2413ForceDisabled && !m_bYM2413CartridgeNotSupported)
